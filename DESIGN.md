...
**Creative North Star: "可探索的算法技术图谱"**

AlgoNote 是一张可以进入练习的算法技术图谱。首页用数据结构、解题范式、迁移关系和真实练习信号组织题库；题目页用代码、思路、复杂度和替代方案表达解题演进。无色偏的内容表面负责可读性，竞赛琥珀标记主路径与关键动作，石墨编辑器承接长时间编码。
...
- 题型关系图是首页的主交互，节点选择会重绘关联路径并更新代表题目
- 最近解题与能力分布使用真实本地练习数据，不使用作品集式技能进度条
...
**Display Font:** IBM Plex Sans Variable（中文回退为 PingFang SC、Microsoft YaHei）
**Body Font:** IBM Plex Sans Variable（中文回退为 PingFang SC、Microsoft YaHei）
**Label/Mono Font:** JetBrains Mono Variable（回退为 Cascadia Code、SFMono-Regular、Consolas）
...
- 桌面端题目面板跨两行，编辑器占据主要宽度并位于右上，190px 结果面板位于右下；面板间距固定为 5px。820px 以下切换为单面板标签流，任何动态文本都不得改变工具栏或标签尺寸。
- 完整思路与解法光谱只在用户主动解锁后出现。光谱通过时间、空间坐标和方案明细比较替代解法，不在初次进入题目时泄露答案。

### Motion

- 图谱节点选择使用 180ms 状态过渡，关联边以方向动画提示迁移关系；平移和缩放由用户直接控制。
- 筛选题型后平滑移动到题目索引；开启减少动态效果时改为即时定位，并停止所有非必要动画。
