...
- 按题号、中英文题名、标签和难度检索
- Monaco Python / C++ 编辑器，本地草稿自动保存；起始模板不包含测试答案
...
进入练习页只显示题面和函数骨架。精选题的检查用例由服务端在运行时附加，不会出现在编辑器源码中。提示、题解、参考实现和 AI 回答都需要用户主动操作；参考实现加载到编辑器后不会覆盖自动保存的个人草稿，可随时点击“返回我的代码”。
